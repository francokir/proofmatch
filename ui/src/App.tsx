...
function App({ api = demoProofMatchUiApi, contractAddress = demoContractAddress, proofStatus }: AppProps) {
  ...
  const startPrivateCheck = async (terms: CandidatePrivateTerms) => { setPrivateTerms(terms); setUiError(''); navigate('proof-progress'); try { await api.prepareCandidatePrivateState(contractAddress, { minimumCompensation: BigInt(Math.trunc(terms.minimumCompensation)), availableWeeklyHours: BigInt(Math.trunc(terms.availableWeeklyHours)) }); if (demoMode) { setDemoProofStatus('proof_generating'); await api.proveMatch(contractAddress); setDemoProofStatus('indexing_pending'); await refreshPublicState(); setDemoProofStatus('confirmed'); } else await api.proveMatch(contractAddress); } catch { setDemoProofStatus('failed'); setUiError('The private check could not be completed. Your private values remain local.'); } };
